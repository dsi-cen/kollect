<section class="container-fluid">
		...
		<div class="col-sm-12 mt-3">
			<div class="card card-body">
				<div class="d-flex justify-content-start">	
					<h1 class="h2">Evolution du nombre de données de <?php echo $nomd;?></h1>
	...
	<div class="row mt-2">
		<div class="col-sm-12">			
			<ul class="nav nav-tabs" id="onglet">
				...
				if($cat == 'oui')
				{
					?><li class="nav-item color4_bg"><a class="nav-link" href="#disc" data-toggle="tab" data-id="disc"><i class="fa fa-file-text-o fa-lg"></i> Nombre par catégorie</a></li><?php
				}
				?>
			</ul>
			<div class="tab-content">
				...
				<div class="tab-pane fade" id="espece">
					...
					<p><a href="index.php?module=liste&amp;action=nouveau&amp;d=<?php echo $nomvar;?>">Détail des nouvelles espèces</a></p>
				</div>
				<?php
				if($cat == 'oui')
				{
					...
					<div class="tab-pane fade" id="disc">
						<h2 class="h4">Evolution du nombre d'espèces par catégories (de <?php echo $anneeune;?> à <?php echo $annéeactuelle;?>)</h2>
						<figure>
	...
	<input id="nbligne" type="hidden" value="<?php echo $nbligne;?>"/><input id="anneeune" type="hidden" value="<?php echo $anneeune;?>"/><input id="nomvar" type="hidden" value="<?php echo $nomvar;?>"/><input id="cat" type="hidden" value="<?php echo $cat;?>"/>
</section>
<script>
...
function graph(choix) {
	'use strict';
	var anneeune = $('#anneeune').val(), nomvar = $('#nomvar').val();
	$.ajax({
		url: 'modeles/ajax/bilan/graphevol.php', type: 'POST', dataType: "json", data: {choix:choix,anneeune:anneeune,nomvar:nomvar},
		success: function(reponse) {
			if (reponse.statut == 'Oui') {
				if (choix = 'espece') { graphespece(reponse.annee,reponse.sp,reponse.newsp); }
				if (choix = 'cat') { graphecat(reponse.annee,reponse.observa); }	
			}			
		}
	});	
}
...
function graphecat(annee,data) {
	...
	$('#graph3').highcharts({		
		...
		credits: {enabled: false},
		xAxis: {categories: annee, title: {text: 'Années'}, labels: {staggerLines: nbligne}},
		yAxis: {title: {text: 'Nombre d\'espèces'}},
		tooltip: {shared: true},
