...
$obser = htmlspecialchars($_GET['d']);
$json_obser = file_get_contents('../json/'.$obser.'.json');
$rjson_obser = json_decode($json_obser, true);
$metakey = $rjson_obser['metakey'];
$nomd = $rjson_obser['nom'];
$nomdeux = $rjson_obser['nomdeux'];
$nomvar = $rjson_obser['nomvar'];
$counomvar = $rjson_obser['couleur'];
...
if (isset ($rjson_site['observatoire']))
{
	...
	{
		$menuobservatoire[] = ['nom'=>$n['nom'],'icon'=>$n['icon'],'var'=>$n['nomvar'],'couleur'=>$n['couleur'],'cat'=>$n['discipline']];
		$menudis[$n['discipline']] = $n['discipline'];
	}
...
$listecomsocial = listecomsocialobserva($nomvar);