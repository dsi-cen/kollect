create materialized view obs.synthese_obs_nflou as
		 ...
		 select_genre AS (
			...
			infos_fiche AS (
             SELECT fiche.idfiche,
                    ...
                    CASE
                        WHEN (cg.geo IS NULL) THEN (
                                ((('{"type":"Feature","properties":{},"geometry":{"type":"Point","coordinates":['::text ||
                                   c_1.lng) || ', '::text) || c_1.lat) || ']}}'::text)
                        ELSE cg.geo
                        END                 AS geom_geojson,
                    c_1.x,
                    c_1.y,
    ...
    SELECT i.idfiche,
           ...
           i.codecom,
           i.commune,
           (i.idsite)::character varying                              AS id_station,
           i.site                                                     AS nom_station,
           (i.lng)::character varying                                 AS lng,
           (i.lat)::character varying                                 AS lat,
           (i.x)::character varying                                   AS x,
           (i.y)::character varying                                   AS y,
           i.codel93 AS maille10x10km,
