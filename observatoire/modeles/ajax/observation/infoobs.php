...
if(isset($_POST['idobs']))
{
	...
	if($obs['statutobs'] != 'No')
	{
		foreach($ligne as $n)
		{
			if($n['denom'] == 'Co' && ($n['tdenom'] == 'IND' || $n['tdenom'] == '' || $n['tdenom'] == 'NSP'))
			{
				...
				{
					$tmpligne[] = '- '.$n['stade'].' ( '.$n['ndiff'].' non différencié(s) & '.$n['male']. ' <i class="fa fa-mars"></i> & '.$n['femelle']. ' <i class="fa fa-venus"></i> )';
				}
				elseif(($n['ndiff'] == 0 || empty($n['ndiff'])) && ($n['male'] != 0 && !empty($n['male'])) && ($n['femelle'] != 0 && !empty($n['femelle'])))
				{
					$tmpligne[] = '- '.$n['stade'].' ( '.$n['male']. ' <i class="fa fa-mars"></i> & '.$n['femelle']. ' <i class="fa fa-venus"></i> )';
				}
				elseif(($n['ndiff'] == 0 || empty($n['ndiff'])) && ($n['male'] == 0 || empty($n['male'])) && ($n['femelle'] != 0 && !empty($n['femelle'])))
				{
					$tmpligne[] = '- '.$n['stade'].' ( '.$n['femelle']. ' <i class="fa fa-venus"></i> )';
				}
				elseif(($n['ndiff'] == 0 || empty($n['ndiff'])) && ($n['male'] != 0 || !empty($n['male'])) && ($n['femelle'] == 0 || !empty($n['femelle'])))
				{
					$tmpligne[] = '- '.$n['stade'].' ( '.$n['male']. ' <i class="fa fa-mars"></i> )';
				}
				elseif(($n['ndiff'] != 0 && !empty($n['ndiff'])) && ($n['male'] == 0 || empty($n['male'])) && ($n['femelle'] == 0 || empty($n['femelle'])))
				{
					$tmpligne[] = '- '.$n['stade'].' ( '.$n['ndiff'].' non différencié(s) )';
				}
				elseif(($n['ndiff'] != 0 && !empty($n['ndiff'])) && ($n['male'] != 0 && !empty($n['male'])) && ($n['femelle'] == 0 || empty($n['femelle'])))
				{
					$tmpligne[] = '- '.$n['stade'].' ( '.$n['ndiff'].' non différencié(s) & '.$n['male']. ' <i class="fa fa-mars"></i> )';
				}
				elseif(($n['ndiff'] != 0 && !empty($n['ndiff'])) && ($n['male'] == 0 || empty($n['male'])) && ($n['femelle'] != 0 && !empty($n['femelle'])))
				{
					$tmpligne[] = '- '.$n['stade'].' ( '.$n['ndiff'].' non différencié(s) & '.$n['femelle']. ' <i class="fa fa-venus"></i> )';
				}
				elseif(($n['ndiff'] == 0 && empty($n['ndiff'])) && ($n['male'] == 0 && empty($n['male'])) && ($n['femelle'] == 0 && empty($n['femelle'])))
				{
					$tmpligne[] = '- '.$n['stade'].' (présent)';
				}				
			}
			...
			elseif($n['denom'] == 'Es' && ($n['tdenom'] == 'IND' || $n['tdenom'] == '' || $n['tdenom'] == 'NSP'))
			{
				if($n['nbmin'] == $n['nbmax'])
				{
					if(($n['ndiff'] != 0 && !empty($n['ndiff'])) && ($n['male'] != 0 && !empty($n['male'])) && ($n['femelle'] != 0 && !empty($n['femelle'])))
					{
						$tmpligne[] = '- '.$n['stade'].' ( environ '.$n['ndiff'].' non différencié(s) & '.$n['male']. ' <i class="fa fa-mars"></i> & '.$n['femelle']. '<i class="fa fa-venus"></i> )';
					}
					elseif(($n['ndiff'] == 0 || empty($n['ndiff'])) && ($n['male'] != 0 && !empty($n['male'])) && ($n['femelle'] != 0 && !empty($n['femelle'])))
					{
						$tmpligne[] = '- '.$n['stade'].' ( environ '.$n['male']. ' <i class="fa fa-mars"></i> & '.$n['femelle']. ' <i class="fa fa-venus"></i> )';
					}
					elseif(($n['ndiff'] == 0 || empty($n['ndiff'])) && ($n['male'] == 0 || empty($n['male'])) && ($n['femelle'] != 0 && !empty($n['femelle'])))
					{
						$tmpligne[] = '- '.$n['stade'].' ( environ '.$n['femelle']. ' <i class="fa fa-venus"></i> )';
					}
					elseif(($n['ndiff'] == 0 || empty($n['ndiff'])) && ($n['male'] != 0 || !empty($n['male'])) && ($n['femelle'] == 0 || !empty($n['femelle'])))
					{
						$tmpligne[] = '- '.$n['stade'].' ( environ '.$n['male']. ' <i class="fa fa-mars"></i> )';
					}
					elseif(($n['ndiff'] != 0 && !empty($n['ndiff'])) && ($n['male'] == 0 || empty($n['male'])) && ($n['femelle'] == 0 || empty($n['femelle'])))
					{
						$tmpligne[] = '- '.$n['stade'].' ( environ '.$n['ndiff'].' non différencié(s) )';
					}
					elseif(($n['ndiff'] != 0 && !empty($n['ndiff'])) && ($n['male'] != 0 && !empty($n['male'])) && ($n['femelle'] == 0 || empty($n['femelle'])))
					{
						$tmpligne[] = '- '.$n['stade'].' ( environ '.$n['ndiff'].' non différencié(s) & '.$n['male']. ' <i class="fa fa-mars"></i> )';
					}
					elseif(($n['ndiff'] != 0 && !empty($n['ndiff'])) && ($n['male'] == 0 || empty($n['male'])) && ($n['femelle'] != 0 && !empty($n['femelle'])))
					{
						$tmpligne[] = '- '.$n['stade'].' ( environ '.$n['ndiff'].' non différencié(s) & '.$n['femelle']. ' <i class="fa fa-venus"></i> )';
					}							
				}
				else
				{
	...
	//$retour['lien'] = $urlval;
	$retour['lien'] = '../index.php?module=observation&action=detail&idobs='.$idobs;
