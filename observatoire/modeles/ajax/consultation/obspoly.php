...
function nbobs($idobser,$observa,$cdnom,$date1,$date2,$typedate,$vali,$photo,$son,$droit,$poly)
{
	$strQuery = 'SELECT COUNT(DISTINCT idobs) AS nb FROM obs.obs INNER JOIN obs.fiche USING(idfiche) INNER JOIN obs.coordonnee USING(idcoord)';
	...
	$strQuery .= " WHERE polygon(path'$poly') @> (lng::text || ',' || lat::text)::point";
	if(!empty($observa)) { $strQuery .= " AND observa IN($observa)"; }
	if(!empty($cdnom)) { $strQuery .= " AND (cdref IN($cdnom))"; }
	if(!empty($idobser)) { $strQuery .= " AND (fiche.idobser = :idobser OR plusobser.idobser = :idobser)"; }
	if(!empty($typedate) && $typedate == 'obs') { $strQuery .= " AND (date1 >= :date1 AND date1 <= :date2)"; }
	if(!empty($typedate) && $typedate == 'saisie') { $strQuery .= " AND (datesaisie >= :date1 AND datesaisie <= :date2)"; }
	if(!empty($vali)) { $strQuery .= " AND (validation = :vali)"; }
...
function listeobs($idobser,$observa,$cdnom,$date1,$date2,$typedate,$vali,$photo,$son,$debut,$droit,$poly)
{
	...
	if(!empty($observa)) { $strQuery .= " AND observa IN($observa)"; }
	if(!empty($cdnom)) { $strQuery .= " AND (cdref IN($cdnom))"; }
	if(!empty($idobser)) { $strQuery .= " AND (observateur.idobser = :idobser OR plusobser.idobser = :idobser)"; }
	if(!empty($typedate) && $typedate == 'obs') { $strQuery .= " AND (date1 >= :date1 AND date1 <= :date2)"; }
	if(!empty($typedate) && $typedate == 'saisie') { $strQuery .= " AND (datesaisie >= :date1 AND datesaisie <= :date2)"; }
	if(!empty($vali)) { $strQuery .= " AND (validation = :vali)"; }
	if($droit == 'non') { $strQuery .= " AND (sensible <= 1 or sensible is null) AND floutage <= 1"; }
...
function nbobscercle($idobser,$observa,$cdnom,$date1,$date2,$typedate,$vali,$photo,$son,$droit,$poly,$lat,$lng)
{
	$strQuery = 'SELECT COUNT(DISTINCT idobs) AS nb FROM obs.obs INNER JOIN obs.fiche USING(idfiche) INNER JOIN obs.coordonnee USING(idcoord)';
	if(!empty($idobser)) { $strQuery .= ' LEFT JOIN obs.plusobser ON plusobser.idfiche = fiche.idfiche'; }
	if($photo == 'oui') { $strQuery .= ' INNER JOIN site.photo USING(idobs)'; }
	if($son == 'oui') { $strQuery .= ' INNER JOIN site.son USING(idobs)'; }
	if($droit == 'non') { $strQuery .= ' LEFT JOIN referentiel.sensible ON sensible.cdnom = obs.cdref'; }
	$strQuery .= " WHERE (6366*acos(cos(radians(:lat))*cos(radians(lat))*cos(radians(lng)-radians(:lng))+sin(radians(:lat))*sin(radians(lat)))) < :dist";
	if(!empty($observa)) { $strQuery .= " AND observa IN($observa)"; }
	if(!empty($cdnom)) { $strQuery .= " AND (cdref IN($cdnom))"; }
	if(!empty($idobser)) { $strQuery .= " AND (fiche.idobser = :idobser OR plusobser.idobser = :idobser)"; }
	if(!empty($typedate) && $typedate == 'obs') { $strQuery .= " AND (date1 >= :date1 AND date1 <= :date2)"; }
...
function listeobscercle($idobser,$observa,$cdnom,$date1,$date2,$typedate,$vali,$photo,$son,$debut,$droit,$poly,$lat,$lng)
{
	$strQuery = "WITH sel AS (SELECT DISTINCT fiche.idfiche, obs.idobs, to_char(date1, 'DD/MM/YYYY') AS datefr, date1, obs.datesaisie, fiche.codecom, fiche.iddep, fiche.idsite, fiche.idobser, validation, observa, obs.cdref, nb, localisation, fiche.floutage, plusobser FROM obs.fiche INNER JOIN obs.obs USING(idfiche) INNER JOIN obs.coordonnee USING(idcoord)";
	if(!empty($idobser)) { $strQuery .= " INNER JOIN referentiel.observateur ON observateur.idobser = fiche.idobser LEFT JOIN obs.plusobser ON plusobser.idfiche = fiche.idfiche"; }
	if($photo == 'oui') { $strQuery .= " INNER JOIN site.photo USING(idobs)"; }
...
if(isset($_POST['choixtax']) && isset($_POST['choixloca']))
{
	...
	$nbobs = ($choixloca == 'poly') ? nbobs($idobser,$observa,$cdnom,$date1,$date2,$typedate,$vali,$photo,$son,$droit,$poly) : nbobscercle($idobser,$observa,$cdnom,$date1,$date2,$typedate,$vali,$photo,$son,$droit,$poly,$_POST['lat'],$_POST['lng']);
	...
	if($nbobs[0] > 0)
	{
		...
		$listeobs = ($choixloca == 'poly') ? listeobs($idobser,$observa,$cdnom,$date1,$date2,$typedate,$vali,$photo,$son,$debut,$droit,$poly) : listeobscercle($idobser,$observa,$cdnom,$date1,$date2,$typedate,$vali,$photo,$son,$debut,$droit,$poly,$_POST['lat'],$_POST['lng']);
