...
function listeobs($idobser,$observa,$cdnom,$codecom,$idsite,$site,$date1,$date2,$typedate,$vali,$photo,$son,$decade,$poly,$dist,$lat,$lng,$etude)
{
	...
	$strQuery .= " ), sel1 AS";
	//$strQuery .= " (SELECT sel.idfiche, idobs, liste.nom, nomvern AS nomfr, sel.codecom, commune, site, iddep, to_char(date1, 'DD/MM/YYYY') AS date1, to_char(date2, 'DD/MM/YYYY') AS date2, CONCAT(sel.observateur, ', ', string_agg(DISTINCT observateur.observateur, ', ')) AS observateur, x, y, lng, lat, codel93, iddet FROM sel";
	$strQuery .= " (SELECT sel.idfiche, idobs, liste.nom, nomvern AS nomfr, sel.codecom, commune, site, iddep, date1, date2, CONCAT(sel.observateur, ', ', string_agg(DISTINCT observateur.observateur, ', ')) AS observateur, x, y, lng, lat, codel93, iddet FROM sel";
	...
	//$strQuery .= " SELECT idobs, sel1.nom, sel1.nomfr, commune, site, date1, date2, sel1.observateur, observateur.observateur AS det, stade.stade, '' AS nb, ndiff, male, femelle, CASE WHEN denom = 'Co' THEN 'Compté' WHEN denom = 'Es' THEN 'Estimé' WHEN denom = 'NSP' THEN 'Non Rens.' END AS denom, typedenom, nbmin, nbmax, etatbio, methode, prospection, statutbio, idfiche, codecom, iddep, x, y, lng, lat, codel93 FROM sel1";
...
if(isset($_POST['choixtax']) && isset($_POST['choixloca']))
{
	...
	{
		$l = null;
		$l .= '<table id="tblexport" class="table table-hover table-sm" cellspacing="0" width="100%">';
		$l .= '<thead><tr>';
		...
		foreach($liste[1] as $n)
		{
			...
			$data[] = [$n[0],$n[1],$n[2],$n[3],$n[4],$tridate,$n[7],$n[8],$n[9],$cnb,$n[11],$n[12],$n[13],$n[14],$n[15],$n[16],$n[17],$n[18],$n[19],$n[20],$n[21],$n[22],$n[23],$n[24],$n[25],$n[26],$n[27],$n[28],$n[29],$n[30]];
