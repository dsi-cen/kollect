...
{
	$fichiertmp = '../../../taxref/taxref'.$_POST['version'].'.csv';
	if(file_exists($fichiertmp)) 
	{
		$fichier = 'taxref'.$_POST['version'].'.csv';
	}
	else
	{
		$retour['statut'] = 'Non';
		$retour['mes'] = '<div class="alert alert-danger" role="alert">Aucun fichier nommé taxref'.$_POST['version'].'.csv.</div>';
