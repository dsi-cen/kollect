...
function nbobs($idobser,$observa,$cdnom,$codecom,$idsite,$site,$date1,$date2,$typedate,$vali,$photo,$son,$droit,$decade,$poly,$dist,$lat,$lng,$etude,$orga,$indice,$statut,$typedon,$flou,$pr,$habitat,$stade,$etatbio,$methode,$prospect,$statbio,$proto,$genre,$famille,$aves,$mort,$maille)
{
	$where = 'non';
	$strQuery = 'SELECT COUNT(DISTINCT idobs) AS nb FROM obs.obs INNER JOIN obs.fiche USING(idfiche) INNER JOIN '.$observa.'.liste USING(cdref)';
	...
	if(!empty($indice)) { $strQuery .= ' INNER JOIN referentiel.liste AS l ON l.cdnom = obs.cdref'; }
...
function listeobs($idobser,$observa,$cdnom,$codecom,$idsite,$site,$date1,$date2,$typedate,$vali,$photo,$son,$debut,$droit,$decade,$poly,$dist,$lat,$lng,$etude,$orga,$indice,$statut,$typedon,$flou,$pr,$habitat,$stade,$etatbio,$methode,$prospect,$statbio,$proto,$genre,$famille,$aves,$mort,$maille)
{
	$where = 'non';
	$strQuery = "WITH sel AS (SELECT DISTINCT fiche.idfiche, obs.idobs, to_char(date1, 'DD/MM/YYYY') AS datefr, date1, obs.datesaisie, fiche.codecom, fiche.iddep, fiche.idsite, fiche.idobser, validation, obs.observa, obs.cdref, nb, localisation, fiche.floutage, plusobser FROM obs.fiche INNER JOIN obs.obs USING(idfiche) INNER JOIN $observa.liste USING(cdref)";
	if($stade != 0 || $etatbio != 'NR' || $methode != 0 || $prospect != 0 || $statbio != 0) { $strQuery .= ' INNER JOIN obs.ligneobs USING(idobs)'; }
	if(!empty($genre)) { $strQuery .= ' INNER JOIN '.$observa.'.genre ON genre.cdnom = liste.cdtaxsup'; }
	...
	$strQuery .= " SELECT sel.idfiche, sel.idobs, sel.datefr, commune, site, sel.iddep, sel.idobser, liste.nom, liste.nomvern, liste.rang, sel.validation, sensible, sel.observa, sel.cdref, nb, localisation, floutage, observateur.nom AS nomobser, prenom, idm, plusobser, nbcom FROM sel";
...
if(isset($_POST['choixtax']) && isset($_POST['choixloca']))
{
	...
	{
		$cdnom = null; $genre = null; $famille = null;
	}	
	if(!empty($choixloca))
	{
	...
	$nbobs = nbobs($idobser,$observa,$cdnom,$codecom,$idsite,$site,$date1,$date2,$typedate,$vali,$photo,$son,$droit,$decade,$poly,$dist,$lat,$lng,$etude,$orga,$indice,$statut,$typedon,$flou,$pr,$habitat,$stade,$etatbio,$methode,$prospect,$statbio,$proto,$genre,$famille,$aves,$mort,$maille);
	...
	if($nbobs[0] > 0)
	{
		...
		$listeobs = listeobs($idobser,$observa,$cdnom,$codecom,$idsite,$site,$date1,$date2,$typedate,$vali,$photo,$son,$debut,$droit,$decade,$poly,$dist,$lat,$lng,$etude,$orga,$indice,$statut,$typedon,$flou,$pr,$habitat,$stade,$etatbio,$methode,$prospect,$statbio,$proto,$genre,$famille,$aves,$mort,$maille);
		...
		foreach($listeobs as $n)
		{
			...
			if($rjson['latin'] == 'oui' && $latin == 'oui') { $afflatin = 'oui'; }
			elseif($rjson['latin'] == 'oui' && ($latin == 'defaut' || $latin == '')) { $afflatin = 'oui'; }
			elseif($rjson['latin'] == 'non' && $latin == 'oui') { $afflatin = 'oui'; }
			elseif($rjson['latin'] == 'non' || $latin == 'non') { $afflatin = 'non'; }
			if($afflatin == 'oui')
			{	
				if($n['rang'] != 'GN')
				{
					$afflatintab = '<a href="index.php?module=fiche&amp;action=fiche&amp;d='.$observa.'&amp;id='.$n['cdref'].'"><i>'.$n['nom'].'</i></a>';
				}
				else
				{
					$afflatintab = '<a href="index.php?module=fiche&amp;action=ficheg&amp;d='.$observa.'&amp;id='.$n['cdref'].'"><i>'.$n['nom'].' sp.</i></a>';
				}
			}
			else
			{
				if($n['nomvern'] != '')
				{
					$afflatintab = '<a href="index.php?module=fiche&amp;action=fiche&amp;d='.$observa.'&amp;id='.$n['cdref'].'" class="tbleu" data-toggle="tooltip" data-placement="top" title="'.$n['nom'].'">'.$n['nomvern'].'</a>';
				}
				else
				{
					if($n['rang'] != 'GN')
			...
			$tabobs[] = ['latin'=>$afflatin, 'taxon'=>$afflatintab, 'vali'=>$clvali, 'tvali'=>$tolvali, 'datefr'=>$n['datefr'], 'nomlat'=>$n['nom'], 'nomfr'=>$n['nomvern'], 'nb'=>$n['nb'], 'loca'=>$localisation, 'afloca'=>$affichagelocalisation, 'obs'=>$obs, 'idobs'=>$n['idobs'], 'idfiche'=>$n['idfiche'], 'flou'=>$n['floutage'], 'com'=>$comobs, 'photo'=>$ouiphoto, 'son'=>$ouison, 'idm'=>$n['idm'], 'plusfiche'=>$plusfiche, 'locadep'=>$locadep, 'idobser'=>$n['idobser']];
