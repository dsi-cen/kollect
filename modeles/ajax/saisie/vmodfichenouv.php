...
function info_fiche($idfiche)
{
	...
	$req = $bdd->prepare("SELECT codecom, decade, idcoord, geo, localisation, CONCAT(fiche.idobser, ', ', string_agg(DISTINCT plusobser.idobser::text, ', ')) AS idobser, idsite, plusobser, fichesup.idfiche AS plusfiche, date1 FROM obs.fiche 
						...
						GROUP BY codecom, decade, idcoord, geo, localisation, fiche.idobser, idsite, plusobser, fichesup.idfiche, date1 ") or die(print_r($bdd->errorInfo()));
...
if(isset($_POST['idcoord']) && isset($_POST['codesite']) && isset($_POST['idfiche'])) 
{
	...
	$iddep = $_POST['iddep'];
	$site = $_POST['site'];
	$rqsite = 'Modification le '.date("d/m/Y");
	$pr = $_POST['pr'];
	...
	$ok = modif_fiche($idfiche,$codecom,$date1mysql,$date2mysql,$decade,$idcoord,$idsite,$obs[0],$iddep,$pr,$floutage,$plusobser,$typedon,$source);
