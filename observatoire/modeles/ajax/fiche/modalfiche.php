...
function nbobsdep($nomvar,$rang,$cdnom,$id)
{
	$bdd = PDO2::getInstance();
	$bdd->query("SET NAMES 'UTF8'");
	if($rang == 'oui')
	{
		$req = $bdd->prepare("SELECT DISTINCT EXTRACT(YEAR FROM date1) AS annee, COUNT(idobs) AS nb FROM obs.fiche
							...
							WHERE (obs.cdref = :cdref OR cdsup = :cdref) AND iddep = :id
	...
	elseif($rang == 'non')
	{
		$req = $bdd->prepare("SELECT DISTINCT EXTRACT(YEAR FROM date1) AS annee, COUNT(idobs) AS nb FROM obs.fiche
							INNER JOIN obs.obs USING(idfiche)
							WHERE cdref = :cdref AND iddep = :id
							GROUP BY annee ") or die(print_r($bdd->errorInfo()));
	}
...
function nbobscom($nomvar,$rang,$cdnom,$id)
{
	$bdd = PDO2::getInstance();
	$bdd->query("SET NAMES 'UTF8'");
	if($rang == 'oui')
	{
		$req = $bdd->prepare("SELECT DISTINCT EXTRACT(YEAR FROM date1) AS annee, COUNT(idobs) AS nb FROM obs.fiche
							INNER JOIN obs.obs USING(idfiche)
							INNER JOIN $nomvar.liste ON liste.cdnom = obs.cdref
							WHERE (obs.cdref = :cdref OR cdsup = :cdref) AND codecom = :id AND (validation = 1 OR validation = 2)
							GROUP BY annee ") or die(print_r($bdd->errorInfo()));
	}
	elseif($rang == 'non')
	{
		$req = $bdd->prepare("SELECT DISTINCT EXTRACT(YEAR FROM date1) AS annee, COUNT(idobs) AS nb FROM obs.fiche
							INNER JOIN obs.obs USING(idfiche)
							WHERE cdref = :cdref AND codecom = :id AND (validation = 1 OR validation = 2)
							GROUP BY annee ") or die(print_r($bdd->errorInfo()));
	}
	$req->bindValue(':cdref', $cdnom);
	$req->bindValue(':id', $id);
...
function nbobsl93($nomvar,$rang,$cdnom,$id)
{
	$bdd = PDO2::getInstance();
	$bdd->query("SET NAMES 'UTF8'");
	if($rang == 'oui')
	{
		$req = $bdd->prepare("SELECT DISTINCT EXTRACT(YEAR FROM date1) AS annee, COUNT(idobs) AS nb FROM obs.fiche
							...
							WHERE (obs.cdref = :cdref OR cdsup = :cdref) AND codel93 = :id AND (validation = 1 OR validation = 2)
							GROUP BY annee ") or die(print_r($bdd->errorInfo()));
	}
	elseif($rang == 'non')
	{
		$req = $bdd->prepare("SELECT DISTINCT EXTRACT(YEAR FROM date1) AS annee, COUNT(idobs) AS nb FROM obs.fiche
							INNER JOIN obs.obs USING(idfiche)
							INNER JOIN obs.coordonnee ON coordonnee.idcoord = fiche.idcoord
							WHERE cdref = :cdref AND codel93 = :id AND (validation = 1 OR validation = 2)
							GROUP BY annee ") or die(print_r($bdd->errorInfo()));
	}
	$req->bindValue(':cdref', $cdnom);
	$req->bindValue(':id', $id);
...
function nbobsutm($nomvar,$rang,$cdnom,$id)
{
	$bdd = PDO2::getInstance();
	$bdd->query("SET NAMES 'UTF8'");
	if($rang == 'oui')
	{
		$req = $bdd->prepare("SELECT DISTINCT EXTRACT(YEAR FROM date1) AS annee, COUNT(idobs) AS nb FROM obs.fiche
							...
							INNER JOIN $nomvar.liste ON liste.cdnom = obs.cdref
							INNER JOIN obs.coordonnee ON coordonnee.idcoord = fiche.idcoord
							WHERE (obs.cdref = :cdref OR cdsup = :cdref) AND utm = :id
							GROUP BY annee ") or die(print_r($bdd->errorInfo()));
	}
	elseif($rang == 'non')
	{
...
if (isset($_POST['id']) && isset($_POST['type']))
{
	...
	if($type == 'com' || $type == 'dep')
	{
		if($type == 'com')
		{
			$coord = (strlen($id) == 5) ? recherche_compoly($id) : recherche_compoly('0'.$id);
			$nbobs = (strlen($id) == 5) ? nbobscom($nomvar,$rang,$cdnom,$id) : nbobscom($nomvar,$rang,$cdnom,'0'.$id);
		}
		elseif($type == 'dep')
		{
			if($id == 1 || $id == 2 || $id == 3 || $id == 4 || $id == 5 || $id == 6 || $id == 7 || $id == 8 || $id == 9)
			{
				...
				$nbobs = nbobsdep($nomvar,$rang,$cdnom,'0'.$id);				
			}
			else
			{
				$coord = recherche_comdep($id);
				$nbobs = nbobsdep($nomvar,$rang,$cdnom,$id);
			}
			$retour['nbobs'] = $nbobs;
		}
	...
	elseif($type == 'maille')
	{
		if($utm == 'non')
		{		
			$nbobs = nbobsl93($nomvar,$rang,$cdnom,$id);
		...
		else
		{
			...
			$nbobs = nbobsutm($nomvar,$rang,$cdnom,$id);
			$resultats = array('crs'=>array('type'=>'name','properties'=>array('name'=>'urn:ogc:def:crs:EPSG::2154')), 'features' => array());
			$feature = array('type' => 'Feature', 'geometry' => Null);
